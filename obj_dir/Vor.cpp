...
void Vor::eval_end_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+eval_end_step Vor::eval_end_step\n"); );
#ifdef VM_TRACE
    // Tracing
...
//============================================================
// Events and timing
bool Vor::eventsPending() { return !vlSymsp->TOP.__VdlySched.empty(); }

uint64_t Vor::nextTimeSlot() { return vlSymsp->TOP.__VdlySched.nextTimeSlot(); }
...
VL_ATTR_COLD void Vor::traceBaseModel(VerilatedTraceBaseC* tfp, int levels, int options) {
    (void)levels; (void)options;
    VerilatedVcdC* const stfp = dynamic_cast<VerilatedVcdC*>(tfp);
    if (VL_UNLIKELY(!stfp)) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vor::trace()' called on non-VerilatedVcdC object;"
            " use --trace-fst with VerilatedFst object, and --trace with VerilatedVcd object");
    }
    stfp->spTrace()->addModel(this);
    stfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vor___024root__trace_register(&(vlSymsp->TOP), stfp->spTrace());
}
